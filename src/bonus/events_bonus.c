...
int	mouse_handler(int button, int x, int y, t_fractal *fractal)
{
	...
	{
		mouse_x = (map(x, -2.0, 2.0, WIDTH) * fractal->zoom)
			+ fractal->shift_x;
		mouse_y = (map(y, 2.0, -2.0, HEIGHT) * fractal->zoom)
			+ fractal->shift_y;
		if (button == Button5)
			fractal->zoom *= 1.05;
		else if (button == Button4)
			fractal->zoom *= 0.95;
		fractal->shift_x = mouse_x - (map(x, -2.0, 2.0, WIDTH)
				* fractal->zoom);
		fractal->shift_y = mouse_y - (map(y, 2.0, -2.0, HEIGHT)
				* fractal->zoom);
	}
	fractal_render(fractal);
	return (0);
